...
def predict_volume(model: torch.nn.Module, image: torch.Tensor, device: torch.device, batch_slices: int = 8, multiple: int = 16):
    ...
    while z < Z:
        ...
        # slices: [N,1,Y,X]
        x = image[:, z:z2].permute(1, 0, 2, 3).contiguous()
...
def main():
    ...
    ap.add_argument("--use_hierarchical_metrics", action="store_true", help="If set, compute hierarchical costs using distance matrix D.")
    ap.add_argument("--save_h_conf", type=str, default=None, help="Optional path to save hierarchical confusion matrix (JSON or NPY). Use {model} to differentiate flat/hier.")
