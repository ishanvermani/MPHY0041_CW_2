...
Use hierarchical loss (!!! ONLY when num_classes=6 !!!!):
        python train.py --data_dir data/preprocessed_data --use_hierarchical_loss --num_classes 6

Key args:
    --data_dir                 according to preprocessed data root containing train/val splits
    ...
    --num_classes              number of segmentation classes (data is cropped to 0..8, default 9)
"""
...
def make_dataloaders(data_root: Path, batch_size: int, num_workers: int,
    ...
    # On Windows, persistent_workers=True can sometimes help speed, 
    # but if it hangs, set num_workers=0
...
def dice_score(preds: torch.Tensor, targets: torch.Tensor, num_classes: int, eps: float = 1e-6) -> float:
    ...
    preds: (B,H,W) int labels; targets: (B,H,W) int labels.
    ...
    for c in range(num_classes):
        ...
        if denom == 0:
            continue  # skip classes absent in both pred and target
...
def train_one_epoch(model, loader, optimizer, device, loss_fn, num_classes, epoch_num):
    model.train()
...
def evaluate(model, loader, device, loss_fn, num_classes, epoch_num):
    model.eval()
    total_loss, total_px, correct_px, total_dice, dice_count = 0.0, 0, 0, 0.0, 0
    
    # Wrap the loader with tqdm for a progress bar
    loop = tqdm(loader, desc=f"Epoch {epoch_num} [Val  ]", leave=False)
...
def main():
    ...
    if args.use_hierarchical_loss:
        if args.num_classes != 6:
            raise ValueError("Hierarchical loss currently defined for num_classes=6. Use --num_classes 6 or disable flag.")
        D = build_distance_matrix(args.num_classes, device)

        def loss_fn(logits, targets):
            # clamp labels into [0, num_classes-1] to avoid out-of-bounds
            if targets.max() >= args.num_classes:
                targets = targets.clamp_max(args.num_classes - 1)
            return hierarchical_ce_loss(logits, targets, D)
    else:
        def loss_fn(logits, targets):
            # safety clamp in case masks contain labels >= num_classes
            if targets.max() >= args.num_classes:
                targets = targets.clamp_max(args.num_classes - 1)
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            ...
            train_loss, train_acc, train_dice = train_one_epoch(
                model, train_loader, optimizer, device, loss_fn, args.num_classes, epoch)
            val_loss, val_acc, val_dice = evaluate(
                model, val_loader, device, loss_fn, args.num_classes, epoch)
            dt = time.time() - t0
